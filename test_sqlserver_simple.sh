...
echo "=== Simple SQL Server Docker Test ==="
echo
...
docker exec sqlserver_test /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P YourStrong!Passw0rd -Q "SELECT @@VERSION"

if [ $? -ne 0 ]; then
    echo "❌ SQL Server is not responding. Let's wait a bit more..."
    sleep 30
    docker exec sqlserver_test /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P YourStrong!Passw0rd -Q "SELECT 1"
    if [ $? -ne 0 ]; then
...
docker exec sqlserver_test /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P YourStrong!Passw0rd -Q "
CREATE DATABASE test_db;
"

docker exec sqlserver_test /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P YourStrong!Passw0rd -d test_db -Q "
-- Create users table
CREATE TABLE users (
